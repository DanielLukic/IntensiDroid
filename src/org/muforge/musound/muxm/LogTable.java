...
public class LogTable
    ...
    private static final int INTERP_SHIFT = MuxmModuleEngine.FP_SHIFT - TABLE_SHIFT;
    ...
    public static int log2( int x )
        ...
        int shift = MuxmModuleEngine.FP_SHIFT;
        while ( x < MuxmModuleEngine.FP_ONE )
            {
            x <<= 1;
            shift--;
            }
        while ( x >= ( MuxmModuleEngine.FP_ONE << 1 ) )
            {
            x >>= 1;
            shift++;
            }
        return ( MuxmModuleEngine.FP_ONE * shift ) + evalTable( tableL2, x - MuxmModuleEngine.FP_ONE );
        }

    /* Raise 2^x. x is in standard fixed point format. */
    public static int pow2( int x )
        {
        int shift = MuxmModuleEngine.FP_SHIFT - ( x >> MuxmModuleEngine.FP_SHIFT );
        int y = evalTable( table2X, x & MuxmModuleEngine.FP_MASK ) << MuxmModuleEngine.FP_SHIFT;
        return y >> shift;
        }
